...
public class FlyingBird extends ApplicationAdapter {
	...
	public void create () {
		...
		bw = Gdx.graphics.getWidth()/12;
		bh = Gdx.graphics.getHeight()/10;
	...
	public void render () {
		...
		if(state==1) {
			...
			font1.draw(batch,"Try again!",0,sh/2);
			...
			font1.draw(batch,"Tap to start!",0,sh/2);
			if(Gdx.input.justTouched()) {
				state = 1;
			}
		}
